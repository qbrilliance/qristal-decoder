...
TEST(DecoderKernelCircuit, simple) {
  ...
  std::vector<int> qubits_beam_metric = {13,14,15,16,17,18};
  std::vector<int> evaluation_bits = {19,20,21,22,23,24};
  ...
  xacc::HeterogeneousMap options{
      ...
      {"total_metric", total_metric},//{"total_metric", qubits_total_metric},
  ...
  for (int i = 0; i < qubits_beam_metric.size(); i++) {
    test_circ->addInstruction(gateRegistry->createInstruction("Measure", qubits_beam_metric[i]));
  }
  ...
  for (int i = 0; i < qubits_metric.size(); i++) {
    test_circ->addInstruction(gateRegistry->createInstruction("Measure", qubits_metric[i]));
  }
  ...
  auto buffer = xacc::qalloc(qubits_string.size() + qubits_metric.size()
                            + qubits_ancilla_adder.size() + qubits_init_null.size()
                            + qubits_init_repeat.size() + qubits_superfluous_flags.size()
                            + qubits_beam_metric.size() + evaluation_bits.size()
                            + qubits_ancilla_pool.size());
